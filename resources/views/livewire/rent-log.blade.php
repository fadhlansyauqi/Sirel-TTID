    ...
    <div class="container mt-2">
		...
		<div class="card card-custom mb-20">
			...
			<div class="card-body ">
				...
				<table class="table table-separate table-head-custom table-checkable" id="kt_datatable_2">
					...
					<tbody>
						@php
							$num = ($rents->currentPage()-1) * $rents->perPage()+1; 
						@endphp
						@foreach($rents as $rent)
							<tr>
								<td>{{ $num++ }}</td>
                                <td>{{ $rent->customerName }}</td>
                                <td>{{ $rent->laptopName }}</td>
                                <td>{{ $rent->rentDate }}</td>
                                <td>{{ $rent->returnDate }}</td>
								<td><span
                                    style="overflow: visible; position: relative; width: 125px;">
                                    <button wire:click="edit('{{ $rent->id }}')" type="button"
                                        class="btn btn-sm btn-clean btn-icon mr-2" title="Edit details"> <span
                                            ...
                                            </svg> </span> </button> <button wire:click="delete('{{ $rent->id }}')"
        ...
        <div class="card card-custom gutter-b example example-compact">
            <div class="card-header">
                <h3 class="card-title">Add Rent Log</h3>
            </div>
            ...
            <form>
                <div class="card-body">
                    ...
                    <div class="form-group">
                        <label for="laptopName">Laptop Name
                            <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" wire:model="laptopName" />
                        @error('laptopName')
                        <div class="mt-2 alert alert-danger" role="alert">
                            {{$message}}
                        </div>
                    ...
                    <div class="form-group">
                        <label for="rentDate">Rent Date
                            <span class="text-danger">*</span></label>
                        <input type="date" class="form-control" wire:model="rentDate" />
                        @error('rentDate')
                        <div class="mt-2 alert alert-danger" role="alert">
                            {{$message}}
                        </div>
